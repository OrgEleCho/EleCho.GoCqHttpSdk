...
public record class CqImageMsg : CqMsg
{
    ...
    internal static string ImageTypeToString(CqImageType? cqImageType) => cqImageType switch
    {
    ...
    internal static CqImageType ImageTypeFromString(string? value) => value switch
    {
    ...
    internal override CqMsgDataModel? GetDataModel() =>
        new CqImageMsgDataModel(Image, CqImageMsg.ImageTypeToString(ImageType), ((int)ImageSubType).ToString(), Url?.ToString(), UseCache.ToInt(), (int?)ImageEffect, ThreadCount);

    internal override void ReadDataModel(CqMsgDataModel? model)
    {
        var m = model as CqImageMsgDataModel ?? throw new ArgumentException("model必须是CqImageMsgDataModel");

        Image = m.file;
        ImageType = CqImageMsg.ImageTypeFromString(m.type);
