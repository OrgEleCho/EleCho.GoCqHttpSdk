...
{
    private static CqMsgModel? MusicMsgModelToObject(JsonDocument doc, JsonSerializerOptions options)
    {
        if (doc.RootElement.TryGetProperty("data", out JsonElement dataEle) && dataEle.TryGetProperty("type", out JsonElement musicTypeEle))
        {
            ...
            {
                "qq" or "163" or "xm" => JsonSerializer.Deserialize<CqMsgModel<CqMusicMsgDataModel>>(doc, options),
                "custom" => JsonSerializer.Deserialize<CqMsgModel<CqCustomMusicMsgDataModel>>(doc, options),
                _ => null,
            };
        }
    ...
    public override CqMsgModel? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (jsondoc.RootElement.TryGetProperty("type", out JsonElement typeEle))
        {
            ...
            return msgType switch
            {
                ...
                Music => CpMsgModelConverter.MusicMsgModelToObject(jsondoc, options),
